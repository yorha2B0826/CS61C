...
class TestAbsLoss(unittest.TestCase):
    def test_simple(self):
        ...
        # load address of `result` into register a3
    ...
    def test_abs_loss_invalid_length(self):
        ...
        t.input_array("a0", array0)
        t.input_array("a1", array1)
        t.input_scalar("a2", -1)  # 负数长度
        t.input_array("a3", result)
        t.call("abs_loss")
        t.execute(code=36)  # 期望以错误码36退出
...
class TestSquaredLoss(unittest.TestCase):
    ...
    def test_squared_loss_length_one(self):
        t = AssemblyTest(self, "../coverage-src/squared_loss.s")
        array0 = t.array([5])
        array1 = t.array([2])
        t.input_array("a0", array0)
    ...
    def test_squared_loss_zero_diff(self):
        t = AssemblyTest(self, "../coverage-src/squared_loss.s")
        array0 = t.array([1, 2, 3])
...
class TestZeroOneLoss(unittest.TestCase):
    def test_simple(self):
        ...
        # check that the result array contains the correct output
        t.check_array(result, [0, 0, 1, 0, 0])
